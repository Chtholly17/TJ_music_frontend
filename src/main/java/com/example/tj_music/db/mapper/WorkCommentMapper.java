...
public interface WorkCommentMapper {
    ...
    @Insert("insert into workComment(work_comment_target, work_comment_owner, work_comment_content) values(#{workCommentTarget}, #{workCommentOwner}, #{workCommentContent})")
    ...
    @Select("select * from workComment where work_comment_target = #{workCommentTarget}")
    public List<WorkComment> selectWorkCommentByTarget(Integer workCommentTarget);

    // delete work comment by work_comment_id
    @Select("delete from workComment where work_comment_target = #{workCommentTarget}")
    public void deleteWorkCommentById(Integer workCommentTarget);

    // delete work comment by work_comment_target
    @Delete("delete from workComment where work_comment_target = #{workCommentTarget}")
